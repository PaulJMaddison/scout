---
name: testing-scout-backend
description: Test the KynticAI Scout open-source backend end-to-end. Use when verifying .NET SDK, API, or script changes.
---

# Testing Scout Backend
...
dotnet test tests/KynticAI.Scout.Sdk.Tests/

# Unit tests
dotnet test tests/KynticAI.Scout.UnitTests/

# Integration tests (uses WebApplicationFactory)
dotnet test tests/KynticAI.Scout.IntegrationTests/

# E2E tests (requires running API; start with start-backend.sh first)
dotnet test tests/KynticAI.Scout.EndToEndTests/
```
...
- `Uri.TryCreate(path, UriKind.Absolute)` behaves differently on Linux vs Windows. Paths starting with `/` are treated as `file:///` URIs on Linux. The fix in `ScoutHttpPipeline.BuildUri()` adds `&& absolute.Scheme != Uri.UriSchemeFile`.
- `realpath -m` (GNU coreutils) may not work on stock macOS — needs a fallback if macOS support is required.
- PowerShell scripts reference `C:\` paths. Bash equivalents use relative sibling paths for private enterprise repositories.
