...
public sealed class LocalLicenceServiceTests
{
    ...
    public async Task Loads_and_verifies_cloud_signed_licence_envelope()
    {
        ...
        var licencePath = Path.Combine(Path.GetTempPath(), $"ucl-cloud-envelope-{Guid.NewGuid():N}.json");
    ...
    public async Task Rejects_cloud_licence_envelope_with_bad_signature()
    {
        ...
        var licencePath = Path.Combine(Path.GetTempPath(), $"ucl-cloud-envelope-bad-{Guid.NewGuid():N}.json");
    ...
    private static ContextLayerDbContext NewDb()
    {
        var options = new DbContextOptionsBuilder<ContextLayerDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString("N"))
            .Options;
        return new ContextLayerDbContext(options);
    }
    ...
    private static string CreateCloudEnvelope(RSA rsa)
    {
        ...
        {
            licenceKey = "UCL-20260513-FICTIONAL",
