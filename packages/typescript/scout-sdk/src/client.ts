...
/**
 * Top-level SDK client exposing all Scout API surfaces.
 *
 * Create an instance with {@link createScoutClient}.
 */
...
export interface TenantScopedScoutClient {
  tenantSlug: string
  users: {
...
class HttpPipeline {
  ...
  private readonly getAccessToken: ScoutClientOptions['getAccessToken'] | undefined
  ...
  constructor(options: ScoutClientOptions) {
    ...
    this.userAgent = options.userAgent ?? 'KynticAI.Scout.TypeScriptSdk/2.0.0'
  ...
  async graphql<T>(operationName: string, query: string, variables: unknown, fieldName: string): Promise<T | null> {
    ...
    if (envelope.errors?.length) {
      ...
      throw new ScoutError(first.message, {
  ...
  private async createError(response: Response): Promise<ScoutError> {
    ...
    try {
      ...
      if (body.error) {
        ...
        return new ScoutError(body.error.message ?? `Request failed with ${response.status}.`, {
      ...
      return new ScoutError(body.detail ?? body.title ?? `Request failed with ${response.status}.`, {
        status: response.status,
      ...
      return new ScoutError(`Request failed with ${response.status}.`, {
        status: response.status,
        correlationId,
      })
    }
 ...
 * ```
 */
export function createScoutClient(options: ScoutClientOptions): ScoutClient {
  ...
  const selectors = {
    async preview(input: PreviewSelectorInput): Promise<SelectorExecutionPreviewResult> {
      ...
      if (!result) {
        throw new ScoutError('Selector preview returned no result.')
      }
    ...
    async validate(input: ValidateSelectorInput): Promise<SelectorValidationResult> {
      ...
      if (!result) {
        throw new ScoutError('Selector validation returned no result.')
      }
  ...
  const recompute = {
    async queueForUser(
      ...
      if (!result) {
        throw new ScoutError('Context recompute returned no result.')
      }
