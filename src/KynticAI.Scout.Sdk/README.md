...
`KynticAI.Scout.Sdk` is the typed .NET SDK scaffold for KynticAI Scout. It wraps the local/private product API behind stable client interfaces so application teams do not need to hand-roll REST and GraphQL calls during pilots.
...
src/KynticAI.Scout.Sdk/
  Abstractions.cs
  ScoutClient.cs
  ScoutClientOptions.cs
  ScoutErrors.cs
  ScoutHttpPipeline.cs
  ScoutModels.cs
  README.md
...
using var scoutConnection = new ScoutClient(new ScoutClientOptions
{
...
var snapshot = await scout.Snapshots.GetLatestForUserAsync("demo", "123");
var snapshotById = await scout.Snapshots.GetByIdAsync("demo", snapshot!.SnapshotId);
...
```csharp
var session = await scout.Auth.LoginAsync(
    new LoginRequest("demo", "admin@scout.local", "DemoAdmin123!"));
```

Machine-to-machine clients can exchange credentials for a scoped bearer token:

```csharp
var token = await scout.Auth.GetMachineTokenAsync(
...
```csharp
var client = new ScoutClient(new ScoutClientOptions
{
    BaseUrl = "http://127.0.0.1:5198",
...
```csharp
var preview = await scout.Selectors.PreviewAsync(
...
```csharp
var queued = await scout.Recompute.QueueForUserAsync("demo", "123", "crm-webhook");
```
...
try
{
    var context = await scout.Users.GetContextAsync("demo", "missing-user");
}
...
- NuGet package id: `KynticAI.Scout.Sdk`
- package output: `src/KynticAI.Scout.Sdk/bin/<Configuration>/`
- symbols: `.snupkg`
