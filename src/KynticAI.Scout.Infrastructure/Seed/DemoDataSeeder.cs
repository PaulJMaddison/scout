...
namespace KynticAI.Scout.Infrastructure.Seed;
...
public static class DemoDataSeeder
{
    ...
    public static async Task SeedAsync(IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
    {
        ...
        await SeedScoutDatabaseAsync(contextDbContext, customerOpsDbContext, passwordHashingService, processor, utcNow, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var product = ProductCatalogItem.Create("CTX-SALES", "Scout Sales Copilot", "revenue-ai", utcNow);
        var plans = new[]
        {
        ...
        foreach (var (scenario, index) in scenarios.Select((item, index) => (item, index)))
        {
            ...
            for (var openIndex = 0; openIndex < emailOpenGoal; openIndex++)
            {
                emailEvents.Add(EmailEngagementEvent.Create(
                    ...
                    "open",
                    "email",
                    Serialize(new { subject = "Scaling Scout across the revenue team" }),
    ...
    private static async Task SeedScoutDatabaseAsync(
        ScoutDbContext contextDbContext,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var opsTenant in opsTenants)
        {
            ...
            {
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "owner@scout.local", "Pat Quinn", passwordHashingService.HashPassword("DemoOwner123!"), OperatorRole.PlatformOwner, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "admin@scout.local", "Dana Mercer", passwordHashingService.HashPassword("DemoAdmin123!"), OperatorRole.TenantAdmin, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "integrations@scout.local", "Riley Chen", passwordHashingService.HashPassword("DemoIntegrations123!"), OperatorRole.IntegrationAdmin, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "analyst@scout.local", "Morgan Lee", passwordHashingService.HashPassword("DemoAnalyst123!"), OperatorRole.Analyst, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "rep@scout.local", "Jordan Kim", passwordHashingService.HashPassword("DemoSales123!"), OperatorRole.SalesUser, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "readonly@scout.local", "Taylor Brooks", passwordHashingService.HashPassword("DemoReadOnly123!"), OperatorRole.ReadOnly, utcNow));
            }
            else
            {
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "admin@summit.scout.local", "Maya Sullivan", passwordHashingService.HashPassword("SummitAdmin123!"), OperatorRole.TenantAdmin, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "integrations@summit.scout.local", "Avery Patel", passwordHashingService.HashPassword("SummitIntegrations123!"), OperatorRole.IntegrationAdmin, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "analyst@summit.scout.local", "Casey Nguyen", passwordHashingService.HashPassword("SummitAnalyst123!"), OperatorRole.Analyst, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "rep@summit.scout.local", "Leo Grant", passwordHashingService.HashPassword("SummitSales123!"), OperatorRole.SalesUser, utcNow));
                operatorAccounts.Add(OperatorAccount.Create(tenant.Id, "readonly@summit.scout.local", "Jamie Walsh", passwordHashingService.HashPassword("SummitReadOnly123!"), OperatorRole.ReadOnly, utcNow));
            }
