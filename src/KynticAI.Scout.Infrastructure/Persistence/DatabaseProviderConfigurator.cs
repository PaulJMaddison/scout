...
internal static class DatabaseProviderConfigurator
{
    ...
    public static void ConfigureScout(DbContextOptionsBuilder optionsBuilder, IConfiguration configuration)
    {
        var connectionString =
            configuration.GetConnectionString("Scout")
            ?? configuration["SCOUT_CONNECTION_STRING"]
            ?? GetDefaultScoutConnectionString();
    ...
    public static void Configure(DbContextOptionsBuilder<ScoutDbContext> optionsBuilder, string? providerName, string connectionString) =>
    ...
    public static string GetDefaultScoutConnectionString() =>
        $"Data Source={GetDefaultDemoDataPath("scout_context_demo.db")}";
