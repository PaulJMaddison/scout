    ...
    : IContextLayerService
{
    ...
    public async Task<AuditEventExportResult> ExportAuditEventsAsync(string tenantSlug, string format, CancellationToken cancellationToken)
    {
        ...
        return normalizedFormat switch
        {
            "csv" => new AuditEventExportResult(
                $"ucl-audit-{tenant.Slug}-{utcNow:yyyyMMddHHmmss}.csv",
                "text/csv",
                BuildAuditCsv(events)),
            "json" => new AuditEventExportResult(
                $"ucl-audit-{tenant.Slug}-{utcNow:yyyyMMddHHmmss}.json",
                "application/json",
    ...
    private async Task<ContextLayer.Domain.Saas.Workspace?> ResolveWorkspaceForEventAsync(
        Guid tenantId,
        string? workspaceSlug,
        CancellationToken cancellationToken)
    {
        ContextLayer.Domain.Saas.Workspace? workspace;
        if (string.IsNullOrWhiteSpace(workspaceSlug))
        {
