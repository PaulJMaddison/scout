...
This page explains how KynticAI Scout runs beside customer systems as a customer-controlled data plane for exact data items, relationships, attribution paths, outcomes, governed context, and local APIs.
...
- exact data items, exact linked records, relationships, attribution paths, outcomes, context snapshots, and governed JSON/context packages
...
By default, raw operational records, connector credentials, customer-specific mappings, exact linked records, relationships, attribution paths, outcomes, context facts, context snapshots, local JSON packages, prompt context packages, message bodies, document bodies, attachments, local databases, and logs with sensitive payloads should not leave the customer-controlled data plane.
...
These records are carried as exact linked records with citation IDs. Deterministic relationships such as email-to-contact, contact-to-account, account-to-opportunity, account/contact-to-activity, contact-to-email-engagement, account/contact-to-web-conversion, account/contact-to-support-ticket, account/contact-to-product-usage, account-to-billing, and account/contact-to-outcome are built in the local data plane. Attribution paths record what happened and in what order. Scout may emit `BasicRelationshipEngine` fallback signals to support a recommended next action and an `EnterpriseRelationshipEngineHandoff` artefact for proof-mode Enterprise analysis. Canonical relationship-set analysis, attribution-path comparison, outcome matching, and governed JSON handoff belong to the Enterprise Rust engine/vector DB. Optional Cloud/control-plane output remains aggregate usage metadata only: counters, status, timestamps, package version, and audit/control-plane event metadata, not relationship types, attribution paths, weights, recommendations, confidence, caveats, citation IDs, raw operational records, or per-entity relationship metadata.
...
4. Relationship sets link data items such as email address, email reply or meeting booked, web conversion, pricing-page visit, account registration/profile, CRM contact, opportunity, support ticket, product usage, billing health, and won/lost sale outcome.
5. Attribution paths record what happened, in what order, for a user, account, workflow, or other business entity.
6. Context snapshots version the facts and local relationship state.
7. Downstream consumers retrieve context through REST, GraphQL, SDKs, internal services, governed JSON packages, or `POST /api/v1/intelligence/next-action`.
8. In the wider UCL direction, private enterprise modules add the Enterprise Rust engine/vector DB before a customer-owned LLM, KynticAI open-source/private LLM runtime, workflow engine, or app explains the ranked next action.
...
The public repo includes the reusable open core: local demo, customer data-plane foundations, APIs, SDKs, generic connectors, mock connectors, selector engine, exact data items, local relationships, provenance, audit, docs, and extension points.

Paid/private scope can include enterprise connector implementations, the proprietary Enterprise Rust engine/vector DB, relationship-set analysis, attribution-path analysis, SSO, SCIM, advanced governance, compliance exports, deployment packs, SLAs, private cloud support, and optional hosted control-plane operations. Those paid implementations are intentionally not shipped in this public repository.
