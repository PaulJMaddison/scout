...
| `09-final-review.md` | Final cross-repo WP2 evidence review covering changes, persisted evidence, test/build results, upgrade path, connector routing, migration tooling, LanceDB import, Cloud entitlement, data boundary, and commit readiness. |
| `investor-summary.md` | Investor-safe WP2 summary with explicit claim limits, Scout to Enterprise/Fortress upgrade explanation, and Cloud data-boundary statement. |
| `customer-safe-summary.md` | Customer-safe explanation of what customers get, how Scout data remains local, how upgrade works, what Cloud controls, what never leaves the environment, and remaining manual steps. |
| `next-work-packages.md` | Recommended WP3, remaining technical gaps, remaining ops/commercial gaps, and suggested prompt list. |
