...
`UclEnterpriseRelationshipEngineHandoffV1` is the proof-mode handoff contract for Enterprise canonical relationship-set analysis. It is generated from local relationship/evidence JSON and stays in the customer-owned data plane. Scout does not call a live Enterprise service in open-core proof mode; it emits candidate relationships, citation provenance, fallback weight scope, and required Enterprise outputs so the Enterprise proof runner can apply canonical relationship-set analysis separately.
...
Scout may include `BasicRelationshipEngine` public fallback weights in the local `UclEvidencePackV1` payload so open-core tests and demos remain executable. Those weights are not canonical. The local pack includes `relationshipWeighting` metadata with `scope: "basic-public-fallback-only"`, `scoutWeightsAreCanonical: false`, and `canonicalOwner: "Enterprise"`.

For proof mode, `EnterpriseRelationshipEngineHandoff` emits `UclEnterpriseRelationshipEngineHandoffV1` beside the local evidence pack. Enterprise consumes that handoff, or the original `UclEvidencePackV1`, as the stable input for the proprietary Rust engine/vector DB to analyse relationship sets, attribution paths, comparable examples, and outcome patterns. Enterprise must not require Cloud to receive local evidence packs or derived customer intelligence.
